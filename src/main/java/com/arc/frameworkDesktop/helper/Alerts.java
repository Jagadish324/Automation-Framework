...
public class Alerts extends CommonHelper {
    ...
    public static void dismissAlert() {
        ...
        AutoWait.alertWait();
        desktopDriver.switchTo().alert().dismiss();
        log.info("End " + Thread.currentThread().getStackTrace()[1].getMethodName());
    }
    ...
    public static String getAlertText() {
        log.info("Start " + Thread.currentThread().getStackTrace()[1].getMethodName());
        AutoWait.alertWait();
        Alert alert = desktopDriver.switchTo().alert();
    ...
    public static boolean isAlertPresent() {
        ...
        List buttons = (List) desktopDriver.executeScript("mobile: alert", args);
        if(buttons.size()>0){
        desktopDriver.switchTo().alert();
            log.info("End " + Thread.currentThread().getStackTrace()[1].getMethodName());
            return true;
        }
        else {
